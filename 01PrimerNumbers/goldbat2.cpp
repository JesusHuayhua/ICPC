...
int main(){
    ...
    for (auto item : list_input){
        ...
        int count = 0;
        vector<int>::iterator endIt,endSubIt;
        endIt = upper_bound(list_prime.begin(),list_prime.end(),item);
        endSubIt = upper_bound(list_prime.begin(),list_prime.end(),item/2);
        int distancia = distance(list_prime.begin(),endSubIt);
        for (unsigned int i = 0; i < list_prime.size(); i++) {
            ...
            if(find(list_prime.begin() + i - 1,endIt,number) != endIt){
